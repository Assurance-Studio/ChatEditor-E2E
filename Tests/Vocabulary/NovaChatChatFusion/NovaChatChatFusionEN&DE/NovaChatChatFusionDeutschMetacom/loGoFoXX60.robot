*** Settings ***
Documentation       Check "LoGoFoXX 60" DEUTSCH vocabulary
...
*** Test Cases ***
Validate "LoGoFoXX 60" vocabulary
    ...
    Select vocabulary    name:"Vorgespeicherte Vokabulare"    name:"LoGoFoXX-Programme"     ${loGoFoXX60}

Assertions for "LoGoFoXX 60 2.0" DEUTSCH vocabulary
    # Click on a button's text - LoGoFoXX 60`
    ...
    # Check to see if the dictionary's name is included in the text box - LoGoFoXX 60
    RPA.Windows.Click    ${edit_mode_btn_english&deutsch}
    ${message}=    RPA.Windows.Get Text    id:65535
    Should Contain    ${message}    "LoGoFoXX 60" ist ein Original vokabular
    RPA.Windows.Click    id:7

    # Check to see if the dictionary's 'name' attribute is its actual name - LoGoFoXX 60
    RPA.Desktop.Press Keys    alt    b
    RPA.Windows.Click    name:"Vorgespeicherte Vokabulare"
    RPA.Windows.Click    name:"LoGoFoXX-Programme"
    ${name}=    RPA.Windows.Get Attribute    name:"LoGoFoXX 60 (aktuell)"    Name
    Should Be Equal    ${name}    LoGoFoXX 60 (aktuell)
